...
<body>
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        <div class="container">
            <a class="navbar-brand" href="#">Diva Ayu Farhani - J3C118066</a>
        ...
        if (isset($_POST['submit'])) {
            ...
            if (mysqli_num_rows($cek) == 0) {
                if (in_array($ekstensi, $ekstensi_diperbolehkan) == true) {
                    if ($ukuran < 900000000000000000000000) {
                        move_uploaded_file($file_tmp, 'file/' . $file_nama);
                        $sql = mysqli_query($koneksi, "INSERT INTO mahasiswa(nim, nama, jenis_kelamin, agama, olah_raga,
                        nama_file) VALUES('$nim', '$nama', '$jenis_kelamin', '$agama', '$olahraga', '$file_nama')") 
                        or die(mysqli_error($koneksi));

                        if ($sql) {
                            echo '<script>alert("Data Berhasil Ditambahkan."); document.location="index.php";</script>';
                        } else {
                            echo '<div class="alert alert-warning">Data Gagal Ditambahkan.</div>';
                        }
                    } else {
                        echo '<div class="alert alert-warning">UKURAN FILE TERLALU BESAR</div>';
                    }
                } else {
                    echo '<div class="alert alert-warning">EKSTENSI FILE YANG DI UPLOAD TIDAK DIPERBOLEHKAN</div>';
                }
            } else {
                echo '<div class="alert alert-warning">NIM tersebut sudah didaftarkan.</div>';
            }
        }
        ?>
